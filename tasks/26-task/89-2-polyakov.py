...
for i in range(1, len(nums)):
    if (
        couples[-1] - nums[i]
    ) >= 3:   # Сравниваем последнее число из массива чисел с разницей >= 3 и текущее число по индексации
...
# Значит, последняя добавленная в `couples` коробка будет максимальное возможной среди всех последних коробок для `couples`.
